...
extern double g_LatestLatencyMs;
...
void FLM_Pipeline::PrintAverageTelemetry(float fFrameLatencyMS)
{
    ...
    g_LatestLatencyMs = (double)m_telemetry.accLatency;
    if (g_hWndOverlay) PostMessage(g_hWndOverlay, WM_USER_UPDATE_LATENCY, 0, 0);
    runningCount++;
}
...
void FLM_Pipeline::PrintOperationalTelemetry(float fFrameLatencyMS, bool bFull)
{
    ...
    // Handling MEASUREMENTS_PER_LINE
    {
        ...
        if (iMeasurementPerLineCounter == m_setting.iNumMeasurementsPerLine)
        {
            ...
            g_LatestLatencyMs = (double)m_telemetry.rowLatency;
            if (g_hWndOverlay) PostMessage(g_hWndOverlay, WM_USER_UPDATE_LATENCY, 0, 0);
            fTotalLineLatencyMS       = 0;
            iMeasurementPerLineCounter = 0;
...
void FLM_Pipeline::ProcessKeyboardCommands()
{
    ...
    // 3. Frame Generation "Hot switch" (Hotkey F8)
    static unsigned char myHotkey[3] = { (unsigned char)VK_F8, 0, 0 }; 

    IF_CONDITION_BECOMES_TRUE( m_keyboard.KeyCombinationPressed(myHotkey) )
    {
        // Перемикаємо режим
        m_runtimeOptions.gameUsesFrameGeneration = !m_runtimeOptions.gameUsesFrameGeneration;
        
        // Виведення в консоль
        if (m_runtimeOptions.gameUsesFrameGeneration) {
            PrintStream("\n[Hotkey F8] Frame Generation: ON\n");
        } else {
            PrintStream("\n[Hotkey F8] Frame Generation: OFF\n");
        }
    }

    // 5. Handle capture loop validation
    ...
    // 6. Handle showing/hiding of settings dialog box on right mouse button click
